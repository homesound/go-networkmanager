...
// and then ask iwgetid to list out the protocol for each of them
...
func (nm *NetworkManager) GetWifiInterfaces() ([]string, error) {
	...
	for _, iface := range ifaces {
		cmd := simpleexec.ParseCmd(fmt.Sprintf("/sbin/iwgetid -p %v", iface))
		...
		if err := cmd.Run(); err != nil {
			...
			log.Debugf("Failed to query interface '%v': %v", iface, err)
		} else {
			if buf.Len() != 0 {
